...
import { MoreVertical, Trash2, Columns, Rows, GripHorizontal, AlertTriangle, X, Check, Palette, Type, Terminal, FileCode, FolderOpen, Globe, Sticker } from 'lucide-react';
...
const PRESETS = [
    { label: 'Terminal', icon: <Terminal size={18} /> },
    { label: 'Code Editor', icon: <FileCode size={18} /> },
    { label: 'File Explorer', icon: <FolderOpen size={18} /> },
    { label: 'Web View', icon: <Globe size={18} /> },
    { label: 'Notes', icon: <Sticker size={18} /> },
];

export const BentoLeaf: React.FC<BentoLeafProps> = ({ node, onSplit, onRemove, onSwap, onColorChange, onContentChange, isRoot }) => {
  ...
  const isSetupMode = node.content === 'New Section';
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 relative overflow-auto">
        {isSetupMode ? (
            <div className="w-full h-full flex flex-col items-center justify-center p-4 bg-white/40">
                <div className="mb-5 text-center">
                    <h3 className="text-sm font-bold text-slate-700 mb-1">Select View</h3>
                    <p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">Pane Configuration</p>
                </div>
                
                <div className="grid grid-cols-3 gap-2.5 mb-5 w-full max-w-[260px]">
                    {PRESETS.map((preset) => (
                        <button 
                            key={preset.label}
                            onClick={() => onContentChange(node.id, preset.label)}
                        ...
                        >
                            <div className="text-slate-400 group-hover/btn:text-blue-600 transition-colors">{preset.icon}</div>
                            <span className="text-[10px] text-slate-600 font-bold group-hover/btn:text-blue-700">{preset.label}</span>
                        </button>
                    ))}
                </div>
