...
type STTTab = 'groq-whisper' | 'deepgram-nova' | 'deepgram-flux';
...
export function SpeechToTextTest() {
  ...
  useEffect(() => {
    ...
    setApiKeys({ groq: groqKey, deepgram: deepgramKey });
...
function STTTabContent({ provider, apiKey }: STTTabContentProps) {
  ...
  const config = useMemo(() => ({
    ...
    sampleRate: 16000,
    encoding: 'linear16',
    chunkingStrategy: provider === 'groq-whisper' ? chunkingStrategy : undefined,
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Groq Whisper Chunking Strategy */}
      ...
      <div className="flex gap-4">
        {/* For Groq Whisper - no connect needed, auto-connected */}
        {provider === 'groq-whisper' ? (
          <>
            {!stt.isRecording ? (
              <button
      ...
      {stt.isRecording && (
        <div className="flex items-center gap-2">
          ...
          <span className="text-success">
            {provider === 'groq-whisper' ? 'Recording...' : 'Recording and transcribing...'}
          </span>
        </div>
      )}

      {stt.isConnected && !stt.isRecording && provider !== 'groq-whisper' && (
