...
const rambleDebug: RambleDebug = {
  ...
  async mergeEntities(targetIdOrName: string, sourceIdOrName: string, newName?: string) {
    ...
    const resolveId = async (idOrName: string): Promise<{ id: string; name: string } | null> => {
      // Try as ID first
      const byId = await entityStore.getById(idOrName)
      if (byId) return { id: byId.id, name: byId.name }
      // Try as name
      const byName = await entityStore.getByName(idOrName)
...
attachDocs(rambleDebug as unknown as Record<string, unknown>, {
  mergeEntities:
    `Merge two entities into one, relinking ALL references across the entire DB (memories, goals, topics, co-occurrences, knowledge tree nodes, timeline events).
